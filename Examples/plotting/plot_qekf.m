...
for i = 1:N
    ...
    R(:,:,i) = X_qekf.Data(1:3,1:3,i);
    q(:,i) = Rotation3d(R(:,:,i)).getEulerAngles;
    v(:,i) = X_qekf.Data(1:3,4,i);
    p(:,i) = X_qekf.Data(1:3,5,i);
    dR(:,i) = X_qekf.Data(1:3,6,i);
    dL(:,i) = X_qekf.Data(1:3,7,i);
    
    % Body Frame Estimates
    vb(:,i) = R(:,:,i)' * v(:,i);
    
    % Parameters
    bg(:,i) = Theta_qekf.Data(i,1:3);
    ba(:,i) = Theta_qekf.Data(i,4:6);
...
ylabel('m')% X_qekf = X_qekf_qekf;
% Theta_qekf = Theta_qekf_qekf;
% plot_results;
